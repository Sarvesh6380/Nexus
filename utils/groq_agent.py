"""
utils/groq_agent.py — Groq LLM client, conflict detection, and AI chat agent.
"""
...
def chat_with_nexus(user_question: str, history: list) -> str:
    """Pull relevant memories → inject as context → ask Groq."""
    groq = get_groq_client()
    if groq is None:
    ...
    context  = (
        "\n".join(f"[{m.get('timestamp','?')}] {m.get('content','')}" for m in memories)
